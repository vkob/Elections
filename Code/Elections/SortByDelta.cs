...
namespace Elections
{
    ...
    public partial class SortByDelta
    {
        ...
        public void StartDominantForRegions(ElectionYear[] electionYears)
        {
            ...
            Action<string, SimpleRow[]> output = (ouputFile, rows) =>
            {
                using (var sw = new StreamWriter(ouputFile, false, Encoding.GetEncoding(1251)))
                {
                    ...
                    sw.WriteLine("</tr>");
                    sw.WriteLine("</table>");
                    sw.WriteLine(Html.WriteToDevTable());
                    sw.WriteLine("</html>");
                }
            };
           ...
           bool calcDiff)
        {
            ...
            var electionsFilePathes = electionYears.Select(ey => Consts.LocalPath +
                                @"\" + Consts.ElectionsDir + @"\" + string.Format(@"{0}\{0}", ey.DirElectionInfo) + ey.Year + ".xml");
            ...
            string uiks = ResultsHtmlLocalElectionsCommitees(electionsLast, lastElectionYear.FooData, electionYears[0]);
            ...
            sbGraphics.AppendFormat("<tr>\n{0}{1}{2}{3}</tr>\n", res.First, res.Second, res.Third, res.Fourth);

            var sbMain = Html.Middle(lastYear, lastElectionYear.ElectionCaption(), indexFiles[(int)Indexes.SortedByAlphabet], mainFoo.EnglishShort, uiks);
        ...
        private Quadruple<string, string, string, string> GenerateDiagrams(
           FooData mainFoo, ElectionYear[] electionYears, FooData[] fooData)
        {
            ...
            return new Quadruple<string, string, string, string>(
           ...
           Dictionary<string, Election>[] electionsAll, ElectionYear[] electionYears, FooData mainFoo)
        {
            ...
            using (var sw = new StreamWriter(htmlFile, false, Encoding.GetEncoding(1251)))
            {
                ...
                sw.WriteLine(Html.WriteToDevTr());
                sw.WriteLine("</table>");
                sw.WriteLine("</html>");
            }
        }
